...
#influence diagram has removed parental links of decision nodes
...
#Numerator States
#states - 0 chance nodes, 1 decision nodes, 2 utility nodes
belief_potential_data = [[Belief_Edge(0, 0, True, True), [0.5, 0.5, 0.5, 0.5], [1, 1]], [Belief_Edge(0, 4, True, True), [0.25, 0.25, 0.25, 0.25], [1, 0]],
                         [Belief_Edge(1, 1, True, True), [0.5, 0.5, 0.5, 0.5], [1, 1]], [Belief_Edge(1, 5, True, True), [0.25, 0.25, 0.25, 0.25], [1, 0]],
                         [Belief_Edge(2, 2, True, True), [0.5, 0.5, 0.5, 0.5], [1, 1]], [Belief_Edge(2, 6, True, True), [0.25, 0.25, 0.25, 0.25], [1, 0]],
                         [Belief_Edge(3, 3, True, True), [0.5, 0.5, 0.5, 0.5], [0, 0]], [Belief_Edge(3, 4, True, True), [0.25, 0.25, 0.25, 0.25], [0, 0]],
                         [Belief_Edge(4, 5, True, True), [0.5, 0.5, 0.5, 0.5], [0, 0]], [Belief_Edge(4, 7, True, True), [0.25, 0.25, 0.25, 0.25], [0, 2]],
                         [Belief_Edge(5, 6, True, True), [0.5, 0.5, 0.5, 0.5], [0, 0]], [Belief_Edge(5, 8, True, True), [0.25, 0.25, 0.25, 0.25], [0, 2]],
                         [Belief_Edge(6, 9, True, True), [0.25, 0.25, 0.25, 0.25], [0, 2]]]            
...
remove_utility = []
for u in belief_potential_data:
    if 2 in u[2]:
        remove_utility.append(u[0].dest)

print(remove_utility)

complete = False
while complete == False:
    all_set = 0
    for n in new_edges:
        if n.src in remove_utility or n.dest in remove_utility:
            new_edges.remove(n)
            all_set = 1
            break
    if all_set == 0:
...
removed_variables = len(remove_utility)
...
print()
print()
for i in range(len(temp_network.adj)):
    for j in range(len(temp_network.adj[i])):
        print(i)
        print(temp_network.adj[i][j].value)
        print(temp_network.adj[i][j].source_state)
        print(temp_network.adj[i][j].destination_state)
        print()
...
actual_length = len(temp_network.adj) - len(remove_utility) 
#vertices = actual_length
...
while (len(temp_edges) - vertices) > 1:
    ...
    temp_network = Graph(temp_edges)
    #vertices = len(temp_network.adj) - len(remove_utility)
    vertices = len(temp_network.adj)
...
variables_list_length = num_variables - len(remove_utility)


while len(included_variables) != variables_list_length:
    common_separator = []
