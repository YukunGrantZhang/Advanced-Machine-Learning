...
def max_product(new_graph, temp_answer):
    global current_rank
    global functions_array
    global answer
    ...
    if current_rank == 1 and len(overall_functions) == len(functions_array):
        answer = temp_answer
        return
    ...
    count = 0
    max_value = 0
    max_value_collection = []
    max_states = []
    max_numerical_states = []
    
    while len(limit_tracker) <= limit and count < n:
        weak_answer = 1
        collection_limit = []
        collection_numerical_limit = []
        
        for x in selected_functions:
            temp_limit = []
            temp_numerical_limit = []
            for y in x[1]:
                if y not in set_graph_states:
                    selection = random.choice([True, False])
                    new_graph.adj[y][0].source_state = selection
                    temp_limit.append(selection)
                    temp_numerical_limit.append(y)
                    
                else:
                    selection = new_graph.adj[y][0].source_state
                    temp_limit.append(selection)
                    temp_numerical_limit.append(y)

            collection_limit.append(temp_limit)
            collection_numerical_limit.append(temp_numerical_limit)
            
        
        if collection_limit not in limit_tracker:
            for x in selected_functions:
                if calculate_function(new_graph, x[0]) > max_value:
                    max_value = calculate_function(new_graph, x[0])
                    max_states = collection_limit
                    max_numerical_states = collection_numerical_limit
                    
                    if max_value in max_value_collection:
                        break
    ...
    temp_answer = temp_answer * max_value
    
    if max_value not in max_value_collection:
        max_value_collection.append(max_value)
    
    answer_states.append(max_states)
    answer_point_states.append(max_numerical_states)
    
    if current_rank == 0:
        answer = temp_answer
        return
    else:
        max_product(new_graph, temp_answer)
